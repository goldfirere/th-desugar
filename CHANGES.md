...
next [????.??.??]
-----------------
* Add `UniqueCounter` and `nextUnique`, and change the implementation of
  `newUniqueName` to use unique counters. This means that the result of
  `newUniqueName` are now deterministic, as they no longer depend on the unique
  numbers generated by Template Haskell (which are inherently
  non-deterministic).
